...
select * from (
select final.*, count(*) over(partition by dim_case_id, end_dt order by eff_dt) as cnt
from final )
where cnt>1


select ref.dim_case_id, ref.dim_managing_org_id, completed_dt, final.dim_case_id
from fact_case_cua_referral ref
left join (
           select * 
            from final
            where final.dim_managing_org_id <> 9
            and final.dim_managing_org_id <> 0
          ) final
on final.dim_case_id = ref.dim_case_id
and final.dim_managing_org_id = ref.dim_managing_org_id
and final.eff_dt = ref.completed_dt
where completed_dt is not null
and called_back_ind = 'N'
and ref.dim_case_id is null
...
select dim_case_id
     ...
     , eff_dt
     , case when trunc(eff_dt) = trunc(end_dt) 
            and cfs2.dim_managing_org_id <> 9 and cfs2.dim_managing_org_id <> 0
            then next_end_dt
            else end_dt end as end_dt
from case_full_slice2 cfs2



select * from
(select final.*, count(*) over(partition by dim_case_id, end_dt order by 1) as cnt
from final )
where cnt > 1