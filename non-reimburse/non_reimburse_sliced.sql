...
, case_one as (     --covers 1 when begin together but end different times  [[________]------] 
    select leads.*
         , 1 as half
      from leads
     ...
     select leads.*
         , 2 as half
      from leads
     where prev_eff_dt = nr_eff_dt and prev_term_dt < nr_term_dt
)
, case_one2 as (
    select case_one.*
         --, prev_term_dt
...
, case_two as (             -- covers 2 when begin at differnt time but ends together [----[_______]]
    select leads.*
         , 1 as half
      from leads
     where nr_term_dt = next_term_dt and nr_eff_dt < next_eff_dt
    union
    select leads.*
         , 2 as helf
      from leads
...
, case_three as (   -- covers case 3 when a different reason begins and ends in the middle of another reason [----[____]----]
    select fh.*
         , 0 as ending
      from first_half2 fh
...
, case_four as (  -- covers case 4 covers when another reason begins during initial reason but ends after initial reason ends [-----[__--]____]
    select fh.*
         , 0 as middle
      from first_half2 fh
     where ( (next_eff_dt > nr_eff_dt and next_eff_dt < nr_term_dt)
