select distinct dir_name, dir_last_name
from
(
select nvl(service_group_name, 'No Service') as service_type, case_name, child_id, child_full_nm, dim_child_id,
birth_dt, age, age_end_month, number_of_months, adm_name, sup_name, wrk_name, visit_status, dhs_cua, dir_worker_id,
nvl(last_visit, 'No Visits') as last_visit, adm_last_name, sup_last_name, wrk_last_name, queue_cd, adopt_dt, dir_name,
case when dir_name like 'CUA%' then substr(dir_name, 1,4) else director_last_name end as dir_last_name, 0 as base_count
from
(
